...
open class RemoteListViewModel @Inject constructor(
	...
	protected open fun createEmptyState(canResetFilter: Boolean) = EmptyState(
		...
		textSecondary = 0,
		actionStringRes = if (canResetFilter) R.string.reset_filter else R.string.remove_source,
	)
	...
	fun removeSource() {
		launchJob(Dispatchers.Default) {
			sourcesRepository.setSourcesEnabled(setOf(source), false)
			onActionDone.call(ReversibleAction(R.string.source_disabled, null))
		}
	}
}

...
class AppSettings @Inject constructor(@ApplicationContext context: Context) {
	...
	var mainNavItems: List<NavItem>
		get() {
			...
			return if (raw.isNullOrEmpty()) {
				listOf(NavItem.HISTORY, NavItem.FAVORITES, NavItem.EXPLORE, NavItem.PROFILE)
			} else {
	...
	val isSuggestionsCarouselExcludeNsfw: Boolean
		get() = prefs.getBoolean(KEY_SUGGESTIONS_CAROUSEL_EXCLUDE_NSFW, false)
	...
	companion object {
		...
		const val KEY_SUGGESTIONS_CAROUSEL_EXCLUDE_NSFW = "suggestions_carousel_exclude_nsfw"
